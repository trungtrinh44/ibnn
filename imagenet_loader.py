...
class HybridTrainPipe(Pipeline):
    def __init__(
        ...
        if torch.distributed.is_initialized():
            ...
            world_size = 1
        self.features = {
            'image/height': tfrec.FixedLenFeature([], tfrec.int64,  0),
            'image/width': tfrec.FixedLenFeature([], tfrec.int64,  0),
            'image/colorspace': tfrec.FixedLenFeature([], tfrec.string,  ""),
            'image/channels': tfrec.FixedLenFeature([], tfrec.int64,  0),
            'image/class/label': tfrec.FixedLenFeature([], tfrec.int64,  0),
        ...
        self.cmnp = ops.CropMirrorNormalize(
            ...
            std=[0.229 * 255, 0.224 * 255, 0.225 * 255] if normalize else [255.0, 255.0, 255.0],
        )
...
class DALIWrapper(object):
    def gen_wrapper(dalipipeline, num_classes, one_hot, memory_format):
        for data in dalipipeline:
            ...
            target = torch.reshape(data[0]["label"], [-1]).cuda().long()-1
            if one_hot:
...
def get_dali_train_loader(dali_cpu=False):
    def gdtl(
        ...
        train_loader = DALIClassificationIterator(
            pipe, reader_name='Train_reader', last_batch_policy=LastBatchPolicy.DROP
        )
        ...
        for next_input, next_target in loader:
            with torch.cuda.stream(stream):
                ...
                if fp16:
                    ...
                    if one_hot:
                        next_target = expand(
                            num_classes, torch.half, next_target)
                else:
                    next_input = next_input.float()
                    if one_hot:
                        next_target = expand(
                            num_classes, torch.float, next_target)
...
def get_pytorch_train_loader(
    ...
    return (
        PrefetchedWrapper(train_loader, start_epoch,
