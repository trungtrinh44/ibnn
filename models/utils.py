...
class StoLayer(object):
    ...
    def spectral_regularizer(self):
        ...
        posterior_mean = posterior_mean - posterior_mean.mean(dim=0, keepdim=True)
        posterior_mean = posterior_mean / posterior_mean.norm(dim=1, keepdim=True, p=2)
        A = posterior_mean @ posterior_mean.T
        return find_singular_value(A) / n_components
