...
def make_sto_layers(cfg, batch_norm=False, n_components=2, prior_mean=1.0, prior_std=1.0, posterior_mean_init=(1.0, 0.75)):
    ...
    for v in cfg:
        if v == "M":
            ...
            conv2d = StoConv2d(in_channels, v, kernel_size=3, padding=1, n_components=n_components,
                               prior_mean=prior_mean, prior_std=prior_std, posterior_mean_init=posterior_mean_init)
            if batch_norm:
                layers += [conv2d, nn.BatchNorm2d(v), nn.ReLU(inplace=True)]
            else:
                layers += [conv2d, nn.ReLU(inplace=True)]
...
class StoVGG(nn.Module):
    def __init__(self, num_classes=10, depth=16, n_components=2, prior_mean=1.0, prior_std=1.0, posterior_mean_init=(1.0, 0.75), batch_norm=False):
        ...
        self.classifier = nn.ModuleList([
            StoLinear(512, 512, True, n_components, prior_mean, prior_std, posterior_mean_init),
            nn.ReLU(True),
            StoLinear(512, 512, True, n_components, prior_mean, prior_std, posterior_mean_init),
            nn.ReLU(True),
            StoLinear(512, num_classes, True, n_components, prior_mean, prior_std, posterior_mean_init)
        ])
