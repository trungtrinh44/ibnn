...
@ex.capture
def data_parallel(module, input, n_sample, indices=None, device_ids=('cuda:0', 'cuda:1'), output_device=None):
    if not device_ids:
        return module(input, n_sample, indices)
    ...
    if indices is None:
        ...
        indices = nn.parallel.scatter(indices, device_ids)
    replicas = replicas[:len(inputs)]
    outputs = nn.parallel.parallel_apply(replicas, [(x, n_sample, ind) for x, ind in zip(inputs, indices)])
    ...
    if model_name.startswith('Sto'):
        ...
        with torch.no_grad():
            for bx, by in test_loader:
                ...
                prob = torch.cat([data_parallel(model, bx, num_test_sample, indices=torch.full((bx.size(0)*num_test_sample,), idx, out=indices, device=bx.device, dtype=torch.long)) for idx in range(model.n_components)], dim=1)
