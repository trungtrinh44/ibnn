...
if __name__ == "__main__":
    ...
    parser.add_argument('--posterior_mean', type=float, default=1.0)
    parser.add_argument('--posterior_std', type=float, default=0.1)
    ...
    text_path = os.path.join(args.root, 'sto_result.json')
    test_loader = get_data_loader(args.experiment, args.batch_size, test_only=True)
    config['model_type'] = 'stochastic'
    ...
    for name, p in model.named_parameters():
        if 'posterior_mean' in name:
            torch.nn.init.constant_(p, args.posterior_mean)
        if 'posterior_std' in name:
            torch.nn.init.constant_(p, args.posterior_std)
            p.data.expm1_().log_()
    ...
    np.save(os.path.join(args.root, 'sto_pred_entropy.npy'), pred_entropy)
