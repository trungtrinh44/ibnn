...
from datasets import imagenet_loader, infinite_wrapper
import models
...
@ex.capture
def get_model(model_name, prior_mean, prior_std, n_components, device, sgd_params, det_params, sto_params, num_epochs, milestones, lr_ratio_det, lr_ratio_sto, posterior_mean_init, posterior_std_init):
    if model_name.startswith('sto'):
        model = getattr(models, model_name)(n_components=n_components, prior_mean=prior_mean, prior_std=prior_std, posterior_mean_init=posterior_mean_init, posterior_std_init=posterior_std_init)
        detp = []
        stop = []
...
@ex.capture
def get_dataloader(train_batch_size, test_batch_size, num_train_workers, num_test_workers):
    return imagenet_loader(train_batch_size=train_batch_size, test_batch_size=test_batch_size, num_train_workers=num_train_workers, num_test_workers=num_test_workers)
...
@ex.automain
def main(_run, model_name, num_train_sample, num_test_sample, device, validation, validate_freq, num_epochs, logging_freq, det_checkpoint):
    logger = get_logger()
    if validation:
    ...
    if det_checkpoint is not None:
        logger.info(f'Load pretrained checkpoint: {det_checkpoint}')
        logger.info(model.load_state_dict(torch.load(det_checkpoint, device), strict=False))
